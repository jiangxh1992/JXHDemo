...
- (void)layoutSubviews {
    ...
    // TODO 修改过的地方
    // 黑框size
    size = CGSizeMake(159, 116);
    
    // 图片
    //indicator.backgroundColor = [UIColor redColor];
    CGRect indicatorF = indicator.bounds;
    indicatorF.origin.y = (bounds.size.height - size.height) / 2 + 25 + yOffset;
    indicatorF.origin.x = (bounds.size.width - size.width) / 2;
    indicatorF.size.width = size.width;
    if (indicatorF.size.height < 40)
    {
        indicatorF.size.height = 40;
    }
    ...
    CGRect labelF;
    labelF.origin.y = CGRectGetMaxY(indicator.frame) + 10;
    labelF.origin.x = (bounds.size.width - size.width) / 2;
    labelF.size = labelSize;
    labelF.size.width = size.width;
    label.frame = labelF;
}
